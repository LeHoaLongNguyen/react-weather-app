...
const Forecast = ({ data }) => {
    ...
    const tomorrowIndex = (WEEK_DAYS.indexOf(currentDay) + 1) % 7;
  
    const forecastDays = WEEK_DAYS.slice(tomorrowIndex, WEEK_DAYS.length).concat(
      WEEK_DAYS.slice(0, tomorrowIndex)
    );
